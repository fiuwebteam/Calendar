...
$(document).ready(function(){
	...
	$("#calendarChooser").change(function() {
		...
		link += "calendars/users/calendar:" + $(this).val();
...
<div class="clear"></div>
	...
	<div class="grid_8">
		<h1 class="titles vPaddingTop_1 vPaddingBottom_0"><?php __('View Calendar Users');?></h1>
	...
	<div class="grid_13 vPaddingTop_1 vPaddingBottom_1">
		<div class="grid_8 alpha">
			 <label class="titles"for="calendarChooser">You are currently viewing the users of calendar </Label>

		 </div>
		<div class="grid_4  omega"><?= $chooser->selectOutput($calendarsList, true); ?></div>
	</div>
...
<tr class="tableHeadings">
	...
	<th>
		<?php echo $paginator->sort('Permission', "CalendarsGroupsUser.group_id");?>
		<?php
			if (isset($this->params["named"]["sort"]) && $this->params["named"]["sort"] == "CalendarsGroupsUser.group_id") {
				if ($this->params["named"]["direction"] == "asc" ) {
					echo $html->image('vertical-arrow-down.gif',array("class" =>"vArrow", "alt" => "Sort Column"));
				} else {
					echo $html->image('vertical-arrow.gif',array("class" =>"vArrow", "alt" => "Sort Column"));
	...
	<th>
		...
		<?php
			if (isset($this->params["named"]["sort"]) && $this->params["named"]["sort"] == "created") {
				if ($this->params["named"]["direction"] == "asc" ) {
					echo $html->image('vertical-arrow-down.gif',array("class" =>"vArrow", "alt" => "Sort Column"));
				} else {
					echo $html->image('vertical-arrow.gif',array("class" =>"vArrow", "alt" => "Sort Column"));
...
?>
	<tr<?php echo $class;?>>
		...
		<td>
			<?php switch ($user['CalendarsGroupsUser']['group_id']) {
				case 1:
					echo "SuperAdmin";
					break;
				case 2:
		...
		<td>
			<?php echo date("M j, Y <b\\r/> g:i a", strtotime($user['User']['created'])) ; ?>
		</td>

		<td class="actions">
		  <ul>
			<li><?php echo $html->link(__('View', true), array('controller'=>'users','action' => 'view', $user['User']['id'])); ?></li>
			
			<?php if ($user["User"]["id"] != 1) { ?>
			<li><?php echo $html->link(__('Edit', true), array('controller' => 'users', 'action' => 'changeTier', "user:{$user["User"]["id"]}" )); ?></li>
			<li><?php echo $html->link(__('Delete', true), array('controller'=>'users','action' => 'delete', $user['User']['id']), null, sprintf(__('WARNING: This will delete the user from all Calendars. Are you sure you want to delete %s?', true), $user['User']['username'])); ?></li>
			<?php } ?>
		  </ul>
		</td>
	</tr>
