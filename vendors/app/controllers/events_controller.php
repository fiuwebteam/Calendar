...
class EventsController extends AppController {
	...
	function index() {
		...
		'noType', 'noView', 'viewType', 'calendar', 'featured', 'calendarName', 'startDate', 'endDate'));
	...
	function month() {
		...
		$start = $lastMonthYear . "-" . $lastMonthMonth . "-" . ($lastMonthDays - $firstWeekdayOfTheMonth );
		$end = date("Y-m-", strtotime("$date +1 month")) . sprintf("%02d",(42 - date("t") - $firstWeekdayOfTheMonth));
		$events = $this->Event->getEvents($calendarId, $start, $end, null, 1000);
		
		$this->set(compact('date', 'lastMonthYear', 'lastMonthMonth', 'lastMonthDays', 
		'firstWeekdayOfTheMonth', 'events', 'nextMonthYear', 'nextMonthMonth', 'calendar', 'calendarName', 'noDate'));
