...
class Event extends AppModel {
	...
	function getCacheKey($page) {
		return $key = sha1($page . "test");
	}
	
	function setCachedPageEvents($page, $events) {
		$memcache = new Memcache();
    	$memcache->connect("localhost") or die ("Could not connect to memcache");
    	$memcache->set($this->getCacheKey($page), $events, 0, 3600);
	}
	
	function getCachedPageEvents($page) {
	...
	*/
	...
	function getCacheKey($conditions, $fields, $order, $recursive) {
		$key = $this->flattenObject($conditions);
		$key .= $this->flattenObject($fields);
		$key .= $this->flattenObject($order);
		$key .= $this->flattenObject($recursive);
		return sha1($key);
	}
	
	function find($conditions = null, $fields = array(), $order = null, $recursive = null) {
		
		$key = $this->getCacheKey($conditions, $fields, $order, $recursive);
    	...
    	if (empty($results)) {
    		...
    		$memcache->set($key, $results, 0, 3600);
    	} 
    	return $results;
	...
	function calendarEventsToSave($eventId, $eventType, $calendars, $repeats) {
		...
		foreach ($repeats as $repeat) {
			...
			if ($repeat["type"] != 1) {
				$stopMarker = date("Y-m-d", strtotime($marker . "+{$repeat["variable"]} $skipValue"));
			} else {
				$stopMarker = $repeat["end_date"];
			}
			
			while($marker <= $stopMarker) {
				$end_time = $start_time = "00:00:00";
				$end_date = $start_date = $marker;
				switch($eventType) {
					// normal
					case 1:
				...
				if (($repeat["type"] == 1 && $eventType == 2 ) || ($repeat["type"] == 2 && $eventType == 2)) {
					$marker = date("Y-m-d", strtotime($end_date . "+1 day"));
				} else  {
					$marker = date("Y-m-d", strtotime($marker . "+1 $skipValue"));
				}
		...
		foreach ($repeats as $repeat) {
			...
			while($marker <= $repeat["end_date"]) {
				...
				switch($eventType) {
					...
					case 3:	
						$start_date_time = "$marker {$repeat["start_time"]}";
						$end_date_time = "$marker {$repeat["start_time"]}";			
						break;
				}
			...
			switch ($repeat["type"]) {
				...
				// every day
				case 1:
					$marker = date("Y-m-d", strtotime($marker . "+1 day"));
					break;
				// weekly
				case 2:
					$marker = date("Y-m-d", strtotime($marker . "+1 week"));
					break;
				// monthly
				case 3:
					$marker = date("Y-m-d", strtotime($marker . "+1 month"));
					break;
			}
	...
	function repeatsToSave($eventId, $formData) {
		...
		$repeat["type"] = $formData["Event"]["repeatsType"];
		
		$repeat["variable"] = $formData["Event"]["repeatVariable"];
		...
		if ( $repeat["type"] == 1 && ($repeat["start_date"] != $repeat["end_date"]) && $formData['timeOptions'] != 3 ) {
			 $repeat["type"] = 2;
		} 
	...
	function cleanEventArray($data) {
		...
		if ($data["Event"]["flyer"]["error"] == 0 && $data["Event"]["flyer"] != "") {
		...
		if ($data["Event"]["thumbnail"]["error"] == 0  && $data["Event"]["thumnail"] != "") {
    ...
    function getEvents($calendarId, $start, $end = null, $categoryId = null, $limit = 0, $featured = 0, $isRss = false) {
    ...
    function calendarEventsToAppend($event, $calendar) {
    	...
    	$cE = $this->CalendarsEvent->find("first", 
    		array("fields" => "calendar_id", "group" => "calendar_id", "recursive" => -1, "conditions" => array("event_id" => $event)));
		$cE = $this->CalendarsEvent->find("all", 
			array("conditions" => array("event_id" => $event, "calendar_id" => $cE["CalendarsEvent"]["calendar_id"] )));
    ...
    function getFeaturedEventsForCalendar($calendarId) {
    	$params = array(
			...
			'conditions' => array(
				"CalendarsEvent.calendar_id" => $calendarId,
				"CalendarsEvent.end_date_time >=" => (date("Y-m-d G")),
    ...
    function getRssEvents($calendarId, $passedParams) {
    	$isRss = ($passedParams["action"] == "rss" || $passedParams["action"] == "json") ? true : false;    	
    ...
    function getIndexPaginationParams($calendarId, $params, $fbsort=false) {
		...
		*/
    	...
    	$conditions["CalendarsEvent.start_date_time <="] = date("Y-m-d", strtotime("today +365 days")) . " 23:59:59";
		$conditions["CalendarsEvent.end_date_time >="] = date("Y-m-d") . " 00:00:00";
		...
		foreach($params as $key => $value) {
			...
			if ($key != "start" && $key != "end" ) { 
				...
				if ($key == "end") {
					$conditions["CalendarsEvent.start_date_time <="] = $value . " 23:59:59"; 
				} else {
					$conditions["CalendarsEvent.end_date_time >="] = $value . " 00:00:00"; 
				}
			}
			if ($key != "type") { 
				$noType[$key] = $value; 
    ...
    function topPermissionForThisEvent($eventId, $userId) {
    	
    	$params = array();
		$params['fields'] = "CalendarsGroupsUser.*, CalendarsEvent.*";
		$params['recursive'] = -1;
		$params['joins'] = array(
			array(
				...
				'type' => 'LEFT',
				'conditions' => array(
					'CalendarsEvent.calendar_id = CalendarsGroupsUser.calendar_id'
				)
			)
		);
		$params['conditions'] = array(
			"OR" => array(
				array(
					...
					"CalendarsGroupsUser.group_id < " => 3
				),
				array(
		...
		$params['order'] = "CalendarsGroupsUser.group_id DESC";
		
		$permissionLvl = $this->CalendarsEvent->find("first",$params);
		
		return $permissionLvl["CalendarsGroupsUser"]["group_id"];
