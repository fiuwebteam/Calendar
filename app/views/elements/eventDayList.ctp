...
if (empty($results)) {	
	...
	while ($mark < $endDate) {
		...
		foreach($otherEvents as $event) {
			if ( $mark ==  date("Y-m-d", strtotime($event['CalendarsEvent']['start_date_time'])) ) {
				...
				for ($x = (count($daySegregation[$mark]) - 1); $x > 1; $x--) {
					if (
						date( "H:i:s", strtotime($daySegregation[$mark][$x]['CalendarsEvent']['start_date_time'])) <
						date( "H:i:s", strtotime($daySegregation[$mark][$x-1]['CalendarsEvent']['start_date_time']))					 
					) {
	...
	$results = array();
	$results["daySegregation"] = $daySegregation;
	$results["ongoingEvents"] = $ongoingEvents;
	$memcache->set($key, $results, 0, 300);
} else {
	$daySegregation = $results["daySegregation"];
	$ongoingEvents = $results["ongoingEvents"];
} 
...
<table class="eventsTable" cellpadding="0" cellspacing="0">
	...
	<tr	<?php echo $class;?>>
		<td class='eventTableDate'>
			<?php
				if (
					$event['CalendarsEvent']['start_date_time'] == $event['CalendarsEvent']['end_date_time'] &&
					$event['CalendarsEvent']['end_date_time'] == "00:00:00" 
				) {
					echo "All day";
				} else {
