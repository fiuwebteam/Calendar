...
<div id="testmodal">Content is here</div>
...
<table id="monthGridTable">
	<tr class="month">
		...
		<span id='currentMonth'> <?= date("F Y", strtotime($date)); $currMonth=date("n", strtotime($date)); echo "cm: ".$currMonth;$curryear=date("Y", strtotime($date));?> </span>
	...
	$thisMonth=0;
	$day = date("Y-m-d", strtotime("$lastMonthYear-$lastMonthMonth-" . ($lastMonthDays - $firstWeekdayOfTheMonth + 1) ));
